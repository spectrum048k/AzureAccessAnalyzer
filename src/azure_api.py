...
class AzureAPI():
    ...
    def get_role_assignments(self, subscription_id):
        ...
        # build the url using base url, subscription_id and api version
        url = f"{self.AZURE_REST_API_BASE_URL}/subscriptions/{subscription_id}/providers/Microsoft.Authorization/roleAssignments?api-version=2022-04-01"
        response = requests.get(url, headers=self.auth_headers)
        return response.json()
    ...
    def get_activity_log(self, subscription_id:str, start_date:datetime, end_date:datetime,
        ...
        url = f"{self.AZURE_REST_API_BASE_URL}/subscriptions/{subscription_id}/providers/microsoft.insights/eventtypes/management/values?api-version=2017-03-01-preview&{filter_param}&{select}"
